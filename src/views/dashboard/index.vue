...
<script>
  ...
  import {updataDashboard,getImgUrls} from '@/api/newLogin'
  let arrImg=[
  ...
  export default {
    ...
    data(){
      return{
        ...
        dataPro:{name:"zaf"},
        // http://localhost:9528/server/public/img/logo.jpg
        imageUrl: ''
    ...
    methods:{
      getImgPath(){
        getImgUrls({name:'zaf'}).then((res)=>{
          console.log('getImgPath',res)
          if(this.imageUrl.indexOf('png')==-1||this.imageUrl.indexOf('jpg')==-1){
            this.imageUrl='';
            this.imageUrl='http://localhost:9528/server'+res.imgList;
          }
